...
<head>
    ...
    <title>Edit Product - Food-Rary</title>
</head>
<body class="bg-[#f0f0f0] min-h-screen font-sans text-black p-8">
    ...
    <div class="max-w-3xl mx-auto bg-white p-8 rounded-lg shadow-[0_0_15px_rgba(0,0,0,0.1)] border-2 border-black">
        <div class="flex items-center gap-3 mb-6 text-3xl font-serif tracking-wider" style="font-variant: small-caps;">
            <h2 class="font-bold border-b-2 border-[#c22026] pb-2 w-full">Edit Product: {{ $product->productName }}</h2>
        </div>
        ...
        <form action="/products/{{ $product->productID }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-2 gap-6 mb-8">
                <div>
                    ...
                    <input type="text" name="productName" value="{{ old('productName', $product->productName) }}" required class="w-full border-2 border-gray-300 rounded p-2 focus:border-black outline-none">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="productPrice" value="{{ old('productPrice', $product->productPrice) }}" required class="w-full border-2 border-gray-300 rounded p-2 focus:border-black outline-none">
                </div>
                <div>
                ...
                <div id="ingredients-container" class="space-y-4">
                    ...
                    @if($oldIngs && is_array($oldIngs))
                        @foreach($oldIngs as $idx => $oldIng)
                            <div class="ingredient-row flex gap-4 items-end mt-4">
                                <div class="flex-1">
                    ...
                    @else
                        @foreach($recipes as $index => $recipe)
                            <div class="ingredient-row flex gap-4 items-end mt-4">
                                <div class="flex-1">
                                    <label class="block font-bold mb-1 text-sm">Ingredient</label>
                                    <select name="ingredients[{{ $index }}][ingredientID]" required class="w-full border-2 border-gray-300 rounded p-2 focus:border-black outline-none">
                                        <option value="" disabled>Select Ingredient...</option>
                                        @foreach($ingredients as $ingredient)
                                            <option value="{{ $ingredient->ingredientID }}" {{ $recipe->ingredientID == $ingredient->ingredientID ? 'selected' : '' }}>
                                ...
                                <div class="w-32">
                                    <label class="block font-bold mb-1 text-sm">Qty Used</label>
                                    <input type="number" step="0.01" name="ingredients[{{ $index }}][qtyUsed]" value="{{ $recipe->qtyUsed }}" required class="w-full border-2 border-gray-300 rounded p-2 focus:border-black outline-none">
                                </div>
                                @if($index > 0)
                                    <button type="button" class="remove-btn bg-red-500 text-white p-2 rounded border-2 border-red-700 hover:bg-red-700 font-bold mb-1 w-10 h-10 flex items-center justify-center">X</button>
                                @endif
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Set the index to the length of existing recipes or old inputs so new ones don't overwrite
            let ingredientIndex = oldCount > 0 ? oldCount - 1 : {{ count($recipes) - 1 }};
            ...
            // Make sure any existing "X" buttons work immediately on page load
